# Specjalizacje — Informatyka PJATK Gdańsk

Zestawienie specjalizacji dla studiów stacjonarnych I stopnia na kierunku **Informatyka**.

> Student wybiera jedną specjalizację na cały tok studiów. Przedmioty specjalizacyjne są realizowane w semestrach 5-7. Każda specjalizacja obejmuje 4 przedmioty po 5 ECTS, łącznie **20 ECTS**.

## Szybkie porównanie

| Specjalizacja | Obszar | Liczba przedmiotów | Łącznie ECTS | Semestry |
|---|---:|---:|---:|---:|
| Aplikacje Internetowe | web, backend, frontend, DevOps | 4 | 20 | 5-7 |
| Cyberbezpieczeństwo | bezpieczeństwo systemów IT | 4 | 20 | 5-7 |
| Inżynieria gier komputerowych | projektowanie i tworzenie gier | 4 | 20 | 5-7 |
| Sztuczna inteligencja | machine learning, deep learning, NLP | 4 | 20 | 5-7 |
| Internet rzeczy | IoT, prototypowanie, systemy czasu rzeczywistego | 4 | 20 | 5-7 |

---

## Aplikacje Internetowe

Specjalizacja skupiona na projektowaniu, tworzeniu i utrzymaniu nowoczesnych aplikacji internetowych — od warstwy backendowej i frontendowej po praktyki DevOps oraz aktualne trendy w obszarze web developmentu.

| Przedmiot | Semestr | Wykład | Laboratorium | Zaliczenie | ECTS |
|---|---:|---:|---:|---|---:|
| Technologie Backend | 5 | 30 | 30 | egzamin | 5 |
| Technologie Frontend | 5 | 30 | 30 | egzamin | 5 |
| Technologie DevOps | 6 | 30 | 30 | egzamin | 5 |
| Trendy w Aplikacjach Internetowych | 7 | 30 | 30 | egzamin | 5 |

> mgr inż. Adam Urbanowicz, mgr Mateusz Miotk

## Cyberbezpieczeństwo

Specjalizacja dla osób zainteresowanych analizą incydentów, testowaniem bezpieczeństwa oraz projektowaniem odpornych architektur IT.

| Przedmiot | Semestr | Wykład | Laboratorium | Zaliczenie | ECTS |
|---|---:|---:|---:|---|---:|
| Kryminalistyka cyfrowa | 5 | 30 | 30 | egzamin | 5 |
| Analiza incydentów cyberbezpieczeństwa | 5 | 30 | 30 | egzamin | 5 |
| Testowanie bezpieczeństwa systemów IT | 6 | 30 | 30 | egzamin | 5 |
| Projektowanie bezpiecznych architektur IT | 7 | 30 | 30 | egzamin | 5 |

> mgr. Andrzej Stojek, mgr. Michał Hyla, mgr. Paweł Lelental

## Inżynieria gier komputerowych

Specjalizacja obejmuje pełny proces tworzenia gier: od prototypowania, przez modelowanie 3D i silniki gier, po projektowanie finalnych rozwiązań.

| Przedmiot | Semestr | Wykład | Laboratorium | Zaliczenie | ECTS |
|---|---:|---:|---:|---|---:|
| Wytwarzanie Gier 1 | 5 | 30 | 30 | egzamin | 5 |
| Modelowanie 3D dla gier | 5 | 30 | 30 | egzamin | 5 |
| Animacje komputerowe | 6 | 30 | 30 | egzamin | 5 |
| Wytwarzanie Gier 2 | 7 | 30 | 30 | egzamin | 5 |

> Aleksander Polin

## Sztuczna inteligencja

Specjalizacja koncentruje się na uczeniu maszynowym, metodach głębokiego uczenia, analizie obrazu oraz przetwarzaniu języka naturalnego.

| Przedmiot | Semestr | Wykład | Laboratorium | Zaliczenie | ECTS |
|---|---:|---:|---:|---|---:|
| Wstęp do nauczania maszynowego | 5 | 30 | 30 | egzamin | 5 |
| Future of Deep Learning | 5 | 30 | 30 | egzamin | 5 |
| Computer Vision | 6 | 30 | 30 | egzamin | 5 |
| Przetwarzanie języka naturalnego | 7 | 30 | 30 | egzamin | 5 |

> dr hab. Jacek Winiarski, dr. Albert Śledzianiowski

## Internet rzeczy

Specjalizacja związana z projektowaniem rozwiązań IoT, szybkim prototypowaniem, systemami czasu rzeczywistego oraz programowaniem platform sprzętowych.

| Przedmiot | Semestr | Wykład | Laboratorium | Zaliczenie | ECTS |
|---|---:|---:|---:|---|---:|
| Szybkie prototypowanie | 5 | 30 | 30 | egzamin | 5 |
| Systemy czasu rzeczywistego | 5 | 30 | 30 | egzamin | 5 |
| Komunikacja i protokoły dla Internetu Rzeczy | 6 | 30 | 30 | egzamin | 5 |
| Programowanie platform sprzętowych | 7 | 30 | 30 | egzamin | 5 |

> dr R. Suski, mgr Michał Hyla, Antoni Ulenberg, Marek Kudła

---

## Podsumowanie organizacyjne

- Każda specjalizacja ma taki sam wymiar: **4 przedmioty, 20 ECTS**.
- Przedmioty specjalizacyjne zaczynają się w **5. semestrze**.
- W 5. semestrze realizowane są **2 przedmioty specjalizacyjne**.
- W 6. i 7. semestrze realizowany jest po **1 przedmiocie specjalizacyjnym**.
- Wszystkie przedmioty specjalizacyjne mają formę: **30 godzin wykładu + 30 godzin laboratorium**.
